...
class DECA(object):
    ...
    def _setup_renderer(self, model_cfg):
        ...
        mask = cv2.cvtColor(cv2.imread(model_cfg.face_eye_mask_path), cv2.COLOR_BGR2RGB).astype(np.float32)/255.; mask = torch.from_numpy(mask[:,:,0])[None,None,:,:].contiguous()
        self.uv_face_eye_mask = F.interpolate(mask, [model_cfg.uv_size, model_cfg.uv_size]).to(self.device)
        mask = cv2.cvtColor(cv2.imread(model_cfg.face_mask_path), cv2.COLOR_BGR2RGB).astype(np.float32)/255.; mask = torch.from_numpy(mask[:,:,0])[None,None,:,:].contiguous()
        self.uv_face_mask = F.interpolate(mask, [model_cfg.uv_size, model_cfg.uv_size]).to(self.device)
        # displacement correction
        fixed_dis = np.load(model_cfg.fixed_displacement_path)
        self.fixed_uv_dis = torch.tensor(fixed_dis).float().to(self.device)
        # mean texture
        mean_texture = cv2.cvtColor(cv2.imread(model_cfg.mean_tex_path), cv2.COLOR_BGR2RGB).astype(np.float32)/255.; mean_texture = torch.from_numpy(mean_texture.transpose(2,0,1))[None,:,:,:].contiguous()
